...
export default function TrackEditor({
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      {/* Header */}
      <div className="px-8 pt-6 pb-5 border-b border-border flex items-start justify-between gap-4 shrink-0">
        <div className="flex items-center gap-3.5">
          ...
          <input
            ...
            className="bg-transparent border-none outline-none text-[22px] font-bold text-text w-full min-w-[200px] placeholder:text-text3"
        ...
        <div className="flex gap-2 shrink-0 items-center">
          <button
            ...
            className="px-3.5 py-[7px] rounded-md text-xs font-medium border border-border2 text-text2 hover:bg-surface2 hover:text-text transition-all whitespace-nowrap"
          >
            AI Suggest
          </button>
          <button
      ...
      <div className="px-8 py-7">
        ...
        <div className="mb-7">
          ...
          <div className="border border-border rounded-[10px] overflow-hidden">
            {/* Table header */}
            <div className="grid bg-surface border-b border-border select-none"
            ...
            {track.exercises.map((ex, ei) => (
              <div
                key={ex.id}
                className="grid border-b border-border last:border-b-0 hover:bg-white/[0.02] transition-colors"
                style={{ gridTemplateColumns: '32px 1fr 120px 40px' }}
              >
                ...
                <input
                  ...
                  className="w-full bg-transparent border-none outline-none px-3.5 py-2.5 text-[13px] text-text"
                />
                <select
                  value={ex.counts}
                  onChange={(e) => updateExercise(ei, { counts: e.target.value })}
                  className="bg-transparent border-none outline-none px-1.5 py-2.5 text-xs text-text2 text-right cursor-pointer appearance-none"
                ...
                <button
                  ...
                  className="flex items-center justify-center text-text3 hover:text-red text-sm transition-colors"
                >
                  &#10005;
                </button>
              </div>
          ...
          <div className="flex gap-2 mt-2.5 items-center">
            <input
              ...
              className="flex-1 bg-surface border border-border2 rounded-md px-3 py-2 text-[13px] text-text outline-none focus:border-white/30 transition-colors placeholder:text-text3"
            />
            <select
              value={newExCounts}
              onChange={(e) => setNewExCounts(e.target.value)}
              className="bg-surface border border-border2 rounded-md px-2.5 py-2 text-xs text-text2 outline-none cursor-pointer"
              style={{ fontFamily: 'DM Mono, monospace' }}
            >
              {COUNTS.map((c) => (
                <option key={c} value={c} className="bg-surface2">
            ...
            <button
              onClick={addExercise}
              className="px-4 py-2 bg-surface2 border border-border2 rounded-md text-xs font-medium text-text2 hover:text-text hover:border-white/25 transition-all whitespace-nowrap"
            >
              + Add
            </button>
          </div>
        ...
        <div className="flex gap-2 pt-1">
          <button
            onClick={onDelete}
            className="px-3.5 py-[7px] rounded-md text-xs font-medium border border-border2 text-text2 hover:border-red hover:text-red hover:bg-red-dim transition-all"
          >
            Delete track
          </button>
          {trackIndex > 0 && (
            <button
              onClick={() => onMove(-1)}
              className="px-3.5 py-[7px] rounded-md text-xs font-medium border border-border2 text-text2 hover:bg-surface2 hover:text-text transition-all"
            >
              &uarr; Move up
            </button>
          )}
