...
export default function AIModal({ action, onClose, onResult, context }: AIModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface border border-border2 rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        {/* Modal header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          ...
          <button
            onClick={onClose}
            className="text-text3 hover:text-text text-lg transition-colors"
          >
            &#10005;
          </button>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
          <button
            onClick={onClose}
            disabled={loading}
            className="px-4 py-2 rounded-md text-xs font-medium text-text2 border border-border2 hover:bg-surface2 hover:text-text transition-all"
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            className="px-5 py-2 rounded-md text-xs font-medium bg-accent text-bg hover:brightness-110 disabled:opacity-50 transition-all flex items-center gap-2"
          >
            {loading && (
